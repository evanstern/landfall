...
// whose predicted drift fits the budget — the big slow oracle when the world
// is calm, a faster one when it is hot, the degrade floor when nothing lands.
// Pure like Route. With no tiers, or none that fit, the returned verdict is
// a suppression carrying the best-quality tier's arithmetic.
func RouteTiered(c Class, velocity float64, tiers []Tier, debt float64) Verdict {
	if len(tiers) == 0 {
		v := Route(c, velocity, 0, debt)
		v.Allow = false
	...
	for i, t := range tiers {
		v := Route(c, velocity, t.SecondsPerPoint, debt)
		if !v.Allow {
			continue
		}
	...
	v := Route(c, velocity, tiers[chosen].SecondsPerPoint, debt)
